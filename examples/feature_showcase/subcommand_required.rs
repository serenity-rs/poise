...
/// This command can only be invoked with `parent child1` and `parent child2`, due to the
/// `subcommand_required` parameter. If you want to allow `parent` to be invoked without a
/// subcommand, remove the `subcommand_required` parameter.
#[poise::command(
    prefix_command,
    slash_command,
